class Cart {
  ...
  constructor(id, product_id) {
    // ssetting up constructor to create an initialize objects of cart class
  ...
  addToPage() {
    ...
    const productName = `${Product.all[this.product_id].product_type}`;
    const productPrice = `${Product.all[this.product_id].price}`;
    item.innerHTML += ` 
      <span class="product-name">${productName} </span>
      <span class="product-price"> ${productPrice} </span>
      <button id=${this.id} class="delete" type="button">R</button>
    `; // calling innerHtml on item to be able to insert the items into the div assigned to each product
    cartItemsClass.appendChild(item);
    document.querySelector(".cart-total-price").innerHTML =
      "$" + (Math.round(Cart.total * 100) / 100).toFixed(2); // added the extra 0 to the total price as well as a dollar signtem); // appending the item to the dom so the user will be able to see products added to cart
  }
}
